...
function contest(){
    global $hand;
    $name = $_POST["content"];
    $result=$hand->query("select id,name,intro,imagesrc,begin,stop from contest_list where name like '%$name%' and status=1");
    $count=0;
...
function team()
{
    ...
    if ($_POST["method"] == 1) {
        ...
        $name = $_POST["content"];
        $sql = "select tid,cid,tcid,peoplenum,name from contest_team where name like '%$name%' and status=1";
        $result = mysqli_query($hand, $sql);
        $num = 0;
        while ($row = mysqli_fetch_assoc($result)) {
            $cid = $row["cid"];
