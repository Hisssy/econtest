...
if($_POST["method"]==1)
{
  ...
  while($row=mysqli_fetch_array($result))
  {
    ...
    while($row2=mysqli_fetch_array($result2))
    {
      $tid=$row2["tid"];
      $tcid=$row2["tcid"];
      $sql3="select uid form contest_join where cid='$cid' and tid='$tid'";
      $result3=mysqli_query($hand,$sql3);
      $row3=mysqli_fetch_array($result3);
      $abc=count($row3);
      $peoplenum=$row2["peoplenum"]-$abc;
      $sql4="select user form account_user where uid='$tcid'";
...
else if($_POST["method"]==2)
{
  ...
  while($row=mysqli_fetch_array($result))
  {
    ...
    $result3=mysqli_query($hand,$sql3);
    $row3=mysqli_fetch_array($result3);
    $sql4="select uid form contest_join where cid='$cid' and tid='$tid'";
    $result4=mysqli_query($hand,$sql4);
    $row4=mysqli_fetch_array($result4);
    $abc=count($row4);
